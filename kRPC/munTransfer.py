...
def engage(connection):
    ...
    neededPhase = (
        2 * math.pi * (1 / (2 * (destSemiMajor**3 / hohmannSemiMajor**3) ** (1 / 2)))
    )
    optimalPhaseAngle = (
        180 - neededPhase * 180 / math.pi
    )  # In degrees; for mun, mun should be ahead of vessel
    ...
    while abs(phaseAngle - optimalPhaseAngle) > 1 or not angleDec:
        ...
        bodyPos = space_center.bodies["Mun"].orbit.position_at(
            space_center.ut, space_center.bodies["Mun"].reference_frame
        )
        ...
        try:
            phaseAngle = math.acos(
                (bodyRadius**2 + vesselRadius**2 - bodyVesselDistance**2)
                / (2 * bodyRadius * vesselRadius)
            )
        except:
    ...
    )  # Get gravitation parameter (GM) for Kerbin
