...
def engage(connection):
    ...
    space_center.warp_to(
        space_center.ut + time_to_warp - 30
    )  # 30 seconds from periapsis
    ...
    while surfaceSpeed() > 1.0:
        ...
        error = (
            vessel.auto_pilot.pitch_error**2 + vessel.auto_pilot.heading_error**2
        ) ** (1 / 2)
        if error > 3:
            vessel.control.throttle = 0
            while error > 1.2:
                error = (
                    vessel.auto_pilot.pitch_error**2
                    + vessel.auto_pilot.heading_error**2
                ) ** (1 / 2)
